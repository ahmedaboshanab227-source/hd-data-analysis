...
# Optimized function using advanced properties from Plotly Trace parameters
def create_bar_chart(df, x_col, y_col, title):
    ...
    # Implementing requested trace customizations for professional 3D looks
    fig.update_traces(
        marker_line_color='#FFCC00',       # Distinct yellow accents border
        marker_line_width=2,               # Solid embossed outline
        textposition='outside',            # Puts numbers clearly on top of bars
        opacity=0.92,                      # Slight high-end glass transparency
        zorder=10,                         # Places bars in front layer
        hovertemplate="<b>%{x}</b><br>Orders Count: %{y}<extra></extra>" # Clean premium tooltips
    )
    return fig
